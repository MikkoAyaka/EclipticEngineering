...
import org.wolflink.minecraft.plugin.eclipticengineering.extension.getDisguisers
...
object DisguiserNotice: Listener {
    ...
    fun on(e: DayNightEvent) {
        if(e.nowTime == DayNightHandler.Status.DAY) {
            val disguisers = getDisguisers()
            if(disguisers.isEmpty()) return
            val randomDisguiser = disguisers.random()
